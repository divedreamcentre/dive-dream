...
import { Phone, Mail, Menu, X, Compass, ChevronRight } from 'lucide-react';
...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      {/* Top Utility bar */}
      <div className="bg-background/95 border-b border-border/40 py-2 text-xs text-muted-foreground hidden md:block">
        <div className="container flex justify-between items-center">
          <div className="flex items-center space-x-6">
            <span className="flex items-center gap-1.5">
              <Phone className="w-3.5 h-3.5 text-primary" />
              <a href="tel:+23057353352" className="hover:text-white transition-colors">+230 5753352</a>
            </span>
            <span className="flex items-center gap-1.5">
              <Mail className="w-3.5 h-3.5 text-primary" />
              <a href="mailto:info@divedreamdivers.com" className="hover:text-white transition-colors">info@divedreamdivers.com</a>
            </span>
          </div>
      ...
      <header className={`sticky top-0 z-50 transition-all duration-300 ${
        isScrolled 
          ? 'bg-background/90 backdrop-blur-md border-b border-border/60 shadow-lg shadow-black/20 py-3' 
          : 'bg-transparent py-5'
      }`}>
        <div className="container flex justify-between items-center">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <div>
              ...
              <span className="text-[9px] tracking-widest text-primary uppercase font-bold block mt-1">Professional Diving Centre</span>
            </div>
          </Link>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 top-[73px] md:top-[109px] z-40 bg-background/98 backdrop-blur-lg lg:hidden border-t border-border/40 overflow-y-auto">
                ...
                return (
                  <Link 
                    ...
                    className={`px-3 py-2.5 rounded-lg text-base font-medium transition-colors ${
                      isActive ? 'bg-primary/10 text-primary border border-primary/20' : 'text-muted-foreground hover:text-white hover:bg-white/5'
                    }`}
                  >
                    {link.label}
                  </Link>
            ...
            <div className="border-t border-border/40 pt-6 flex flex-col space-y-4">
              ...
              <div className="flex flex-col items-center space-y-2 text-sm text-muted-foreground">
                <span className="flex items-center gap-2">
                  ...
                  <a href="tel:+23057353352">+230 5753352</a>
                </span>
                <span className="flex items-center gap-2">
                  <Mail className="w-4 h-4 text-primary" />
                  <a href="mailto:info@divedreamdivers.com">info@divedreamdivers.com</a>
                </span>
              </div>
            </div>
          </div>
      ...
      {/* Premium Footer */}
      <footer className="bg-background border-t border-border/40 pt-20 pb-8 relative overflow-hidden">
        ...
        <div className="container">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-16">
            ...
            <div className="flex flex-col space-y-6">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                Dive Dream Diving Centre Ltd. offers professional SDI/TDI training and guided diving expeditions in Mauritius since 2004. Experience over 40 pristine dive sites with our certified instructors and elite crew.
              </p>
              <div className="flex items-center space-x-4">
                <span className="text-white bg-primary/10 px-2.5 py-1 rounded border border-primary/20 text-[10px] font-semibold uppercase tracking-wider">SDI 5-Star</span>
                <span className="text-white bg-primary/10 px-2.5 py-1 rounded border border-primary/20 text-[10px] font-semibold uppercase tracking-wider">TDI Facility</span>
              </div>
            </div>

            {/* Explore Links */}
            <div className="flex flex-col space-y-4">
              <h4 className="text-base font-semibold text-white tracking-wide">Explore</h4>
              <ul className="space-y-2.5 text-sm text-muted-foreground">
                <li><Link href="/dive-sites" className="hover:text-white transition-colors flex items-center gap-1"><ChevronRight className="w-3.5 h-3.5 text-primary" /> Dive Sites</Link></li>
                <li><Link href="/courses" className="hover:text-white transition-colors flex items-center gap-1"><ChevronRight className="w-3.5 h-3.5 text-primary" /> Courses</Link></li>
                <li><Link href="/packages" className="hover:text-white transition-colors flex items-center gap-1"><ChevronRight className="w-3.5 h-3.5 text-primary" /> Dive Packages</Link></li>
                <li><Link href="/promotions" className="hover:text-white transition-colors flex items-center gap-1"><ChevronRight className="w-3.5 h-3.5 text-primary" /> Promotions</Link></li>
                <li><Link href="/services" className="hover:text-white transition-colors flex items-center gap-1"><ChevronRight className="w-3.5 h-3.5 text-primary" /> Services</Link></li>
                <li><Link href="/reservations" className="hover:text-white transition-colors flex items-center gap-1"><ChevronRight className="w-3.5 h-3.5 text-primary" /> Book a Dive</Link></li>
              </ul>
            </div>

            {/* Information Links */}
            <div className="flex flex-col space-y-4">
              <h4 className="text-base font-semibold text-white tracking-wide">Information</h4>
              <ul className="space-y-2.5 text-sm text-muted-foreground">
          ...
          <div className="border-t border-border/40 pt-8 flex flex-col md:flex-row justify-between items-center text-xs text-muted-foreground gap-4">
            <div>
              &copy; {new Date().getFullYear()} AquaDepth Dive Center. All rights reserved. Professional SDI & TDI 5-Star Facility.
            </div>
            <div className="flex space-x-6">
              <a href="#" className="hover:text-white transition-colors">Privacy Policy</a>
              <a href="#" className="hover:text-white transition-colors">Terms of Service</a>
              <a href="#" className="hover:text-white transition-colors">Safety Liability</a>
            </div>
          </div>
        </div>
      </footer>
